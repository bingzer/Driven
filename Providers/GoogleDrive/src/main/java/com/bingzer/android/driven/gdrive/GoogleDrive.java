    ...
    class SharingImpl extends AbsSharing {
        ...
        public boolean removeSharing(RemoteFile remoteFile, String user) {
            try{
                ...
                PermissionList list = getGoogleDriveApi().permissions().list(remoteFile.getId()).execute();
                for(Permission permission : list.getItems()){
                    if(user.equalsIgnoreCase(permission.getValue())){
                        getGoogleDriveApi().permissions().delete(remoteFile.getId(), permission.getId());
                        return true;
                    }
                }
                return false;
